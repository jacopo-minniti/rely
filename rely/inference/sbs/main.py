...
from rely.inference.sbs.strategies import SamplingStrategy, UniformStrategy, PumStrategy, PumPerValueStrategy, TokenEntropyStrategy
...
def main():
    ...
    parser.add_argument("--strategy", type=str, default="uniform", choices=["uniform", "pum", "pum_per_value", "token_entropy"], help="Sampling strategy to use.")
    ...
    if args.strategy in ['pum', 'pum_per_value']:
        if not args.uncertainty_model_path or args.uncertainty_model_gpu is None:
            parser.error("--uncertainty_model_path and --uncertainty_model_gpu are required for --strategy in ['pum', 'pum_per_value']")
