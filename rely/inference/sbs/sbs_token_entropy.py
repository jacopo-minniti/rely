...
class StepBeamSearch:
    ...
    def _calculate_token_entropy(self, log_probs_data) -> float:
        """Calculate mean entropy over tokens using top-k vocabulary from logprobs."""
...
def _sbs_worker(args: argparse.Namespace, dataset_slice: List[Dict[str, Any]], rank: int, value_task_queue: Queue, value_result_queue: Queue):
    ...
    sbs_config = SBSConfig(
        ...
        value_method=args.value_method, entropy_k=args.entropy_k,
    )
...
def main():
    ...
    parser.add_argument("--entropy_k", type=int, default=20, help="Top-k tokens to consider for entropy calculation.")
