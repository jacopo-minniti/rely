...
class GuidedTreeSearch:
    ...
    def _get_scores_from_server(self, queue_task, queue_result, texts: List[str]) -> List[float]:
        """Send a batch of texts to a model server and get the aggregated scores."""
        if not texts:
            return []
        request_id = str(uuid.uuid4())
        queue_task.put((request_id, self.worker_rank, texts))
        while True:
            response_id, results = queue_result.get()
    ...
    def search(self, user_question: str, system_prompt: str = MATH_SYSTEM_PROMPT) -> tuple[list[Branch], list[Branch], int]:
        ...
        # Score the initial step
        value_scores = self._get_scores_from_server(self.value_task_queue, self.value_result_queue, [first_node_text])
        value_score = value_scores[0] if value_scores else 0.0
        ...
        while tokens_used < self.config.budget:
            ...
            # Batch uncertainty scoring
            uncertainty_texts = [branch.text for branch in leaf_nodes_to_expand]
            uncertainty_scores = self._get_scores_from_server(self.uncertainty_task_queue, self.uncertainty_result_queue, uncertainty_texts)
            for branch, u_score in zip(leaf_nodes_to_expand, uncertainty_scores):
                branch.uncertainty = u_score

            newly_generated_candidates = []
            budget_exceeded = False
            
            for branch in leaf_nodes_to_expand:
                num_branches_to_gen = self.uncertainty_to_branches(branch.uncertainty, self.config.uncertainty_threshold, self.config.max_branches)
                ...
                for step_text, new_tokens in generated_steps:
                    ...
                    candidate = Branch(
                        ...
                        step_count=branch.step_count + 1, score=0, uncertainty=branch.uncertainty, # Value will be added in batch
                        value=0, total_tokens=total_tokens, id=branch_id_counter, parent_id=branch.id
                    )
            ...
            # Batch value scoring
            value_texts = [c.text for c in newly_generated_candidates]
            value_scores = self._get_scores_from_server(self.value_task_queue, self.value_result_queue, value_texts)
            for candidate, v_score in zip(newly_generated_candidates, value_scores):
                candidate.value = v_score
                candidate.score = v_score # or some combination
            ...
            selection_pool = [b for b in all_branches if b.id not in parent_ids and not b.final_answer]

            selection_pool.sort(key=lambda b: b.value, reverse=True)
            leaf_nodes_to_expand = selection_pool[:self.config.beam_width]
