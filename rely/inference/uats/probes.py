...
def load_probes(
    ...
    try:
        ...
        uncertainty_probe.load_state_dict(torch.load(uncertainty_probe_path, map_location=device))
        print(f"Successfully loaded uncertainty probe as MLPProbe from {uncertainty_probe_path}")
    except Exception as e:
        raise RuntimeError(
            f"Failed to load uncertainty probe from {uncertainty_probe_path}: {e}"
        )
