"""
Generate completions from JSONL data using vLLM.

This module provides functions for generating multiple completions from JSONL data
...
def create_cut_cot(attempt: str) -> str:
    """
    Create cut_cot by splitting the attempt by "\n\n" and sampling a random step.
    Returns all CoT up to that step.
    
    Args:
        attempt: The full attempt text
        
    Returns:
        The cut chain-of-thought text
    """
    if not attempt:
        return ""
    
    # Split by "\n\n" to get individual reasoning steps
    steps = attempt.split("\n\n")
    
    if len(steps) <= 1:
        return attempt
    
    # Sample a random step (excluding the last step to ensure we have some CoT)
    max_step = len(steps) - 1
    if max_step > 0:
        sampled_step = random.randint(0, max_step)
    else:
        sampled_step = 0
    
    # Take all steps up to the sampled step
...
def load_jsonl_data(file_path: str) -> Optional[List[Dict[str, Any]]]:
    """
    ...
    Returns:
        List of data items or None if loading failed
    """
    data = []
    try:
        ...
        print(f"ERROR: Failed to load JSONL file at {file_path}: {e}")
        return None


def save_jsonl_data(data: List[Dict[str, Any]], file_path: str) -> bool:
    """
    ...
    try:
        ...
        print(f"ERROR: Failed to save JSONL file at {file_path}: {e}")
        return False


def complete_from_jsonl(
    input_file: str,
    output_file: str,
    ...
    """
    Generate completions from JSONL data using vLLM with data parallelism.
    ...
    if args.dp_size > 1:
        ...
        if exit_code == 0:
            _merge_jsonl_output_files(args)
        else:
        ...
        os.rename(f"{base}.rank0.jsonl", args.output)
        print(f"Processing complete! Output written to {args.output}")


def _complete_jsonl_worker(args, global_dp_rank: int, local_dp_rank: int, master_addr: str, master_port: int) -> None:
    """
    Worker function for data parallel JSONL completion generation.
    ...
    all_data = load_jsonl_data(args.input)
    if all_data is None:
        return
    ...
    for item in data_chunk:
        try:
            # Create cut_cot from attempt field
            attempt = item.get("attempt", "")
            cut_cot = create_cut_cot(attempt)
            ...
            if not question or not cut_cot:
                print(f"Skipping item on rank {global_dp_rank}: missing question or cut_cot")
                continue
    ...
    base, ext = os.path.splitext(args.output)
    output_file_path = f"{base}.rank{global_dp_rank}.jsonl"
    processed_count = 0
    output_data = []
    for source_info, output_group in zip(source_metadata, all_outputs):
        completions = [out.text for out in output_group.outputs]
        
        # Create output item with all original fields plus cut_cot and completions
    ...
    save_jsonl_data(output_data, output_file_path)
...
def _merge_jsonl_output_files(args) -> None:
    """Merges the output files from each rank into a single JSONL file."""
    print("Merging output files...")
    base, ext = os.path.splitext(args.output)
    merged_data = []
    for rank in range(args.dp_size):
        rank_file = f"{base}.rank{rank}.jsonl"
        try:
            part_data = load_jsonl_data(rank_file)
            if part_data is not None:
                merged_data.extend(part_data)
            else:
                print(f"Warning: {rank_file} could not be loaded, skipping.")
                continue
    ...
    save_jsonl_data(merged_data, args.output)
    print(f"Final merged output written to {args.output}")


def main():
    """Command-line interface for complete_from_jsonl."""
    parser = argparse.ArgumentParser(description="vLLM Data Parallel Inference for Uncertainty Probe Data")
    # Application Arguments
    parser.add_argument("--input", type=str, default="generations-mmlu-qwen3-1.7B.jsonl", help="Input JSONL file path")
    parser.add_argument("--output", type=str, default="100-short-completions-mmlu-qwen3-1.7B-v2.jsonl", help="Output JSONL file path")
    ...
    parser.add_argument("--dp-size", type=int, default=8, help="Total data parallel size (number of ranks across all nodes)")
    parser.add_argument("--tp-size", type=int, default=1, help="Tensor parallel size (number of GPUs per process)")
    parser.add_argument("--gpu-memory-utilization", type=float, default=0.92, help="GPU memory utilization fraction")
